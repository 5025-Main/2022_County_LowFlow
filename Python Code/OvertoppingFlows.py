...
def get_weir_dims(site_name,weir_dims):
    vnotch_in= weir_dims.loc[site_name,['h2_height of vnotch']].values
    width_in = weir_dims.loc[site_name,['b2_width of vnotch']].values + weir_dims.loc[site_name,['c1_top crest left']].values + weir_dims.loc[site_name,['c2_top crest right']].values
...
def CTRSC_compound_weir(site_name,WL,weir_dims,plot_timeseries=False,plot_scatter=False):
    ...
    b1 = float(weir_dims.loc[site_name,['b1_left side rectangle']].values[0]) + float(weir_dims.loc[site_name,['b1_right side']].values[0])
